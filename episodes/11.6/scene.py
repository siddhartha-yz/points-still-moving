...
# D2L 11.6.1.2–11.6.1.3 valley: f(x) = 0.1 x₁² + 2 x₂², start [-5, -2].
# Same η for both beats so the only new symbol is β.
...
class Episode116(Scene):
    """A ~32-second silent momentum-versus-GD visualization."""
    ...
    def construct(self) -> None:
        print("D2L 11.6  f(w)=0.1 w1**2 + 2 w2**2")
        ...
        print("η = {:.2f}  β = {:.2f}  momentum path:".format(ETA, BETA))
        for index, (weights, value) in enumerate(zip(PATH_MOM, F_MOM)):
            print(
                "  k={:d}  w=[{:+.5f}, {:+.5f}]  f={:.6f}".format(
                    index, weights[0], weights[1], value
                )
            )
        print("D2L 11.6 last f(w) GD={:.6f}  momentum={:.6f}".format(float(F_GD[-1]), float(F_MOM[-1])))

        # 0.00–0.40 s: chapter mark only.
        chapter_mark = Text("11.6", font_size=66, color=WHITE)
        ...
        w_glyph.add_updater(lambda mob: mob.move_to(traveler_center() + np.array([0.40, 0.34, 0.0])))
        ...
        def f_prefix_anchor() -> np.ndarray:
            """Ahead of the trail during GD; below the traveler near the origin."""
            center = traveler_center()
            if w1_tracker.get_value() > -1.15:
        ...
        mom_formula_v = Text("v ← βv + ∇f", font_size=32, color=WHITE).move_to(np.array([1.55, 3.38, 0.0]))
        mom_formula_w = Text("w ← w − ηv", font_size=32, color=WHITE).move_to(np.array([1.55, 2.88, 0.0]))
        beta_label = Text("β = 0.5", font_size=24, color=YELLOW).move_to(
            eta_label.get_center() + np.array([0.0, -0.50, 0.0])
        )
        ...
        mom_segments = [
            Line(self.plot_point(*PATH_MOM[index]), self.plot_point(*PATH_MOM[index + 1])).set_stroke(
                YELLOW, width=3.6, opacity=0.98
            )
            for index in range(STEPS_MOM)
        ]
        ...
        self.play(
            FadeIn(mom_formula_v),
            FadeIn(mom_formula_w),
            FadeIn(beta_label),
            run_time=0.70,
            rate_func=smooth,
        )
        ...
        # Beat 2: momentum damps the narrow-axis bounce and runs down the valley.
        for index, segment in enumerate(mom_segments):
            self.play(
                Create(segment),
                w1_tracker.animate.set_value(float(PATH_MOM[index + 1, 0])),
                w2_tracker.animate.set_value(float(PATH_MOM[index + 1, 1])),
                run_time=0.70,
                rate_func=smooth,
            )
        self.wait(0.80)
        ...
        self.play(final_hold.animate.set_value(1.0), run_time=8.20, rate_func=linear)
